...
const Transport: React.FC = () => {
  ...
  const { status, message, data, offset, limit, count, taxiData, taxiStatus, taxiMessage } = useSelector(
  ...
  return (
    <div className="container pt-[140px] pb-80">
      <h1 className="title-2 color-black text-[#000000] slt:text-[34px] slt:leading-[40px]">Transport</h1>
      <nav className="hidden lt:block my-50 slt:mt-30">
        <ul className="flex justify-center gap-[50px]">
          <li>
            ...
            {nav === "cars" && <div className="rounded-[6px] h-[6px] bg-[#3C3C3C] "></div>}
          </li>
          <li>
            <button
              onClick={() => setNav("taxi")}
            ...
            {nav === "taxi" && <div className="rounded-[6px] h-[6px] bg-[#3C3C3C] "></div>}
          </li>
        </ul>
      </nav>
      <div className="mt-80 flex justify-between gap-[30px] lt:justify-center">
        {(isLt && nav !== "cars") || (
          <section className="flex-[0_1_845px]">
            <h2 className="text-[36px] leading-[42px] font-normal lt:hidden">Car rental</h2>
            <StatusCheck status={status} errorMsg={message} loadingStyles="min-h-[50vh]">
        ...
        {(isLt && nav !== "taxi") || (
          <section ref={aside} className="relative flex-[0_1_405px] lt:flex-[0_1_685px]">
            <div
              className={`max-w-[405px] lt:max-w-none ${
                sticky === "sticky" ? "fixed top-[20px]" : sticky === "bottom" ? "absolute bottom-0" : ""
              }`}
            >
              <h2 className="text-[36px] leading-[42px] font-normal lt:hidden">Taxi</h2>
              <StatusCheck status={taxiStatus} errorMsg={taxiMessage}>
                ...
                <Warning>If you need another route, please contact us. We will try to find a taxi for you</Warning>
              </StatusCheck>
            </div>
          </section>
        )}
