...
const Blog: React.FC = () => {
  ...
  const { id } = useParams()
  const { data, status, errorMsg } = useSelector((state: RootState) => state.blogNewsAbout)

  useEffect(() => {
    window.scrollTo({ top: 0 })
    id && dispatch(getBlogNewsAbout({ id: +id, similarBlogsLimit: 3, category: "Blog" }))
  }, [id])
      ...
      <div className="pt-[120px] pb-80 lt:pb-40">
        ...
        >
          <div className="container z-10">
            <h1 className="text-[48px] leading-[56px] font-medium text-white text-start">{data.title}</h1>
            <h2 className="my-10 text-[24px] leading-[28px] font-medium">Blog</h2>
            <span>{data.created_at?.split("-").join(".")}</span>
          </div>
        </div>
        <div className="pt-80 container lt:pt-40">
          ...
          <div className="mt-80">
            ...
            <div className="mt-40 flex gap-[30px]">
              {data.similar?.length ? (
                data?.similar?.map((blog: IBlogNews, index: number) => <BlogCard {...blog} type="normal" key={index} />)
              ) : (
