...
const Photo: React.FC = () => {
  ...
  return (
    <>
      <Accordeon title={<h2 className="pl-10 title-2 smbl:pl-0">Photo</h2>} className="pt-30">
          ...
          <ul className="pb-20 grid grid-cols-2 gap-[10px] bottom-line tb:grid-cols-1">
            {data.photos?.images.map((photo, key) => (
              <li className="max-h-[295px]" key={key}>
                <button onClick={() => openPhoto(key)} className="w-full h-full">
      ...
      {isPhotoOpen && (
        ...
        >
          <div className="relative flex justify-center gap-[70px] ldt:gap-[25px]" onClick={(e) => e.stopPropagation()}>
            <button className="absolute top-0 right-0" onClick={() => setIsPhotoOpen(false)}>
              <img src={cross} alt="cross" />
            </button>
            {photo >= 1 ? (
              <button className="w-[40px] flex-shrink-0 slt:hidden" onClick={() => setPhoto((id) => id - 1)}>
