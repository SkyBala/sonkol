...
const Pagination: React.FC<PaginationProps> = ({ page, limit, change, initialVisiblePages = 3, className = "" }) => {
  const symbol = "..."
  const [pages, setPages] = useState<(number | string)[]>([])

  const createNumbersArray: (min: number, max: number) => number[] = (min, max) => {
    const numbers = []
  ...
  useEffect(() => {
    ...
    if (limit < 4) {
      ...
      currPages = [...createNumbersArray(1, initialVisiblePages), symbol, limit]
    }
  ...
  const handlePage = (newPage: number | string) => {
    ...
    setPages((pages) => {
      ...
      if (typeof newPage === "number") {
        if (newPage >= initialVisiblePages && newPage <= limit - initialVisiblePages + 1) {
        ...
        if (newPage > limit - initialVisiblePages) {
          return [1, symbol, ...createNumbersArray(limit - initialVisiblePages + 1, limit)]
        }
      }
  ...
  return (
    <ul className={`flex items-center gap-[4px] outline-none ${className}`}>
      ...
      {pages.map((item, key) => (
        <li key={key}>
          <button
            ...
            style={{
              backgroundColor: item === symbol ? "inherit" : item === page ? "#205871" : "#F3F3F3",
      ...
      <button className="ml-[20px] flex-shrink-0" onClick={goNext}>
        <img src={page + 1 <= limit ? rightArrow : rightArrowDsb} alt="right-arrow" />
      </button>
    </ul>
  )
}
