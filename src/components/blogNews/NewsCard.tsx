...
const NewsCard: React.FC<IBlogNews> = ({ id, title, created_at, image }) => {
  return (
    <div className="max-w-[405px] w-full text-start dt:max-w-[380px] lt:max-w-[690px] lt:mx-auto">
      <h3 className="mb-[5px] text-[24px] leading-[28px] text-[#0B0B0B] lt:mb-[7px]">{title}</h3>
      <span className="text-[#232323]">{created_at.split("-").join(".")}</span>
      ...
      <Link className="block text-blue text-18 leading-[24px] text-end" to={`/news/${id}`}>
        More details...
      </Link>
    </div>
  )
