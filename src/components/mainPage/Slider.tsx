    ...
    return (
      <section className="h-[720px] relative text-white ldt:h-[calc(100vh+400px)] ldt:max-h-[930px]">
        {data.map((tour) => (
          <div
            style={{
              backgroundImage: `url(${tour?.tours[0]?.image})`,
        ...
        <div className="absolute top-[265px] left-0 w-full z-[100] h-[calc(100%_-_265px)] ldt:h-full ldt:top-0 ldt:pb-80">
          <div className="h-full container flex justify-between gap-[30px] ldt:flex-col ldt:items-center">
              ...
              {data
                ...
                .map((tour) => (
                  <div key={tour.id}>
                    ...
                    <span className="text-[18px] leading-[21px] ldt:block ldt:text-center">{tour.description}</span>
                    <div className="mt-20 flex gap-[40px] ldt:justify-center slt:gap-[20px]">
                      <Link className="btn slt:px-[24px]" to={`/tour/${tour.id}`}>
                        Learn more
                      </Link>
                      <Link className="btn slt:px-[24px] transparent" to={`/tours`}>
                        More tours
                      </Link>
                    </div>
                  </div>
            ...
            >
              {(data.length >= 2 ? [...data, ...data] : data).map((tour, index) => (
                <SwiperSlide key={`${tour.id}${index}`} data-id={tour.id}>
                  ...
                  >
                    <h3 className="mb-10 text-16 leading-[18px] font-normal z-10">{tour.name}</h3>
