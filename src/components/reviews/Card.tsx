...
const Card: React.FC<IReview> = ({ name, stars, text, photos, created_at }) => {
    ...
    <div>
      ...
      <div className="mt-[8px] mb-[22px] flex gap-[10px] items-center">
        {[...new Array(5)].map((_, key) => (
          <img key={key} src={stars - (key + 1) >= 0 ? starActive : star} alt="star" className="w-[24px] h-[24px]" />
        ))}
        <span className="text-blueLight text-[24px] leading-[28px] font-medium">{stars}</span>
      </div>
      <p className="whitespace-normal overflow-hidden text-ellipsis">{text}</p>
      <Swiper breakpoints={swiperImgBreakpoints} slidesPerView={1} spaceBetween={10} className="mt-10 mb-20" loop>
        {(photos.length >= 3 ? [...photos, ...photos] : photos).map((image, key) => (
          <SwiperSlide
            key={key}
      ...
      <span className="block text-end text-[14px] leading-[16px] text-[#707070]">{created_at}</span>
    </div>
  )
}
