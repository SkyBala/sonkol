...
import { Sort, setOffset, setSortBy, setTour } from "../../store/slices/reviewsSlice"
import Pagination from "../ui/Pagination"

const Content: React.FC = () => {
  const dispatch = useAppDispatch()
  const tours = useSelector((state: RootState) => state.tours.data)
  const { data, sortBy, tour, count, offset, limit } = useSelector((state: RootState) => state.reviews)
  ...
  const handleSortValue = (newValue: string) => {
    dispatch(setSortBy(sortOptionsValue[sortOptions.findIndex((value) => value === newValue)]))
  }
  ...
  return (
    <div className="container mt-[120px]">
      <div>
        <h3 className="mb-[8px] text-[16px] leading-[18px] text-smooth">Sorting</h3>
        <Select
          options={sortOptions}
          value={sortOptions[sortOptionsValue.findIndex((value) => value === sortBy)]}
      ...
      <div className="mt-20">
        <h3 className="mb-[8px] text-[16px] leading-[18px] text-smooth">Tour</h3>
        <Select
      ...
      <div className="mt-40 [&>:not(:last-child)]:mb-[77px]">
        {data.length ? (
          data.map((review) => <Card key={review.id} {...review} />)
        ) : (
